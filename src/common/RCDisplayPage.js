...
const RCDisplayPage = ({
  ...
  if (dataToPaginate) {
    const indexOfLastItem = externalCurrentPage * itemsPerPage;
    ...
    const totalPages = externalTotalPages || Math.ceil(dataToPaginate.length / itemsPerPage);

    for (let i = 1; i <= totalPages; i++) {
      paginationItems.push(
        <PaginationItem key={i} active={i === externalCurrentPage}>
