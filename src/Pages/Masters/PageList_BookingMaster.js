...
const PageList_BookingMaster = () => {
  const [gridData, setGridData] = useState([]);
  const [loading, setLoading] = useState(true);
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const breadCrumbTitle = "Booking Master";
  const breadcrumbItem = "Booking Master";
  const rtPage_Add = "/AddBooking";
  const rtPage_Edit = "/EditBooking";
  const API_URL = API_WEB_URLS.MASTER + "/0/token/BookingMaster";
...
export default PageList_BookingMaster;
