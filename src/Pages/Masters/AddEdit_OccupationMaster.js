...
function AddEdit_OccupationMaster() {
  const API_URL_SAVE = `${API_WEB_URLS.OccupationMaster}/0/token`;  
  const API_URL_EDIT = `${API_WEB_URLS.MASTER}/0/token/OccupationMasterId/Id`;  
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (error && error.message && error.message.toLowerCase().includes('already exists')) {
        alert("The Occupation is already Exists");
      } else {
        alert("The Occupation is already Exists");
      }
    }
...
export default AddEdit_OccupationMaster;
